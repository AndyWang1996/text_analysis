...
def init_board(N):
    X = np.array([(round(random.uniform(-1, 1), 3), round(random.uniform(-1, 1), 3)) for i in range(N)])
    return X
...
# parse_output(out)
# data = init_board(15)
data = np.array([[0, -0.8], [0, -0.7], [0, -0.75], [-0.1, -0.8], [-0.15, -0.77],
                 [-0.75, 0.75], [-0.75, 0.85], [-0.85, 0.77], [-0.8, 0.73], [-0.825, 0.8],
                 [0.75, 0.75], [0.78, 0.78], [0.75, 0.73], [0.72, 0.8], [0.7, 0.81]])
print(data)
k = 0
while(k < 20):
    out = find_centers(list(data), 3)
